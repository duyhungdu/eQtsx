<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="List.ascx.cs" Inherits="xRP.Administrator.QuanLyHeThong.ChucVu.List" %>
...
<script type="text/javascript">
    function deleteItem(id) {
        ...
        $.ajax({
            type: "POST",
            url: "<%=Util.SERVER_NAME %><%=RootPageUrl%>Default.aspx/DeletePostionItem",
            data: "{\"itemID\":'" + id + "'}",
    ...
    function doSearch() {
        ...
        var url = "";
        if (key.length == 0) {
            url = "Default.aspx?page=position";
        } else {
            url = "Default.aspx?page=position" + "&s=" + key;
        }
    ...
    function OpenModal(id, text) {
        LoadUserControl("<%=Util.SERVER_NAME %>/Default.aspx", "/Administrator/QuanLyHeThong/ChucVu/AddOrEdit.ascx", "&id=" + id);
        ...
        <div class="right-hub-content">
            <div class="work-item-form" style="display: block;">
                <div class="workitem-info-bar">
                    <div class="info-text-wrapper"><span class="info-text">Danh sách chức vụ</span></div>
                </div>
                <div class="toolbar workitem-tool-bar" style="display: block;">
                    <ul class="menu-bar">
                        <li class="menu-item icon-only" id="btn-tinh-chat-xuat-nhap">
                            <a onclick="OpenModal('', 'Thêm chức vụ');" href="javascript:void(0);">
                                <span class="icon icon-add"></span><span class="text">Thêm chức vụ</span>
                            </a>
                        </li>
                            ...
                            <asp:Repeater ID="rptData" runat="server">
                                <ItemTemplate>
                                    <tr>
                                        ...
                                        <td><%# Eval("GhiChu") %></td>
                                        <td class="center-col"><%#Util.RerenderStatus(Eval("KichHoat"),"Đã kích hoạt","Chưa kích hoạt") %></td>
                                        <td class="center-col">
                                        ...
                                        <td class="center-col">
                                            ...
                                            <a class="edit-icon" id="edit-icon" href="javascript:OpenModal('<%# Eval("ID")%>','Sửa: <%# Eval("TenChucVu") %>');"></a>
                                            
<%--                                            
                                            <a class="edit-icon" id="edit-icon" href="default.aspx?page=addposition&id=<%# Eval("ID")%>"></a>--%>
