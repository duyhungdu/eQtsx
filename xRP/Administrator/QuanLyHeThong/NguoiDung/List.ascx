<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="List.ascx.cs" Inherits="xRP.Administrator.QuanLyHeThong.NguoiDung.List" %>
...
<script type="text/javascript">
    function deleteItem(id) {
        ...
        $.ajax({
            type: "POST",
            url: "<%=Util.SERVER_NAME %><%=RootPageUrl%>Default.aspx/DeleteUserItem",
            data: "{\"itemID\":'" + id + "'}",
    ...
    function doSearch()
    {
        ...
        if (key.length == 0)
        {
            url = "Default.aspx?page=user";
        } else {
            url = "Default.aspx?page=user" + "&s=" + key;
        }
...
<script type="text/javascript">
    function OpenModal(id, text) {
        LoadUserControl("<%=Util.SERVER_NAME %>/Default.aspx", "/Administrator/QuanLyHeThong/NguoiDung/AddOrEdit.ascx", "&id=" + id);
        ...
        <div class="right-hub-content">
            <div class="work-item-form" style="display: block;">
                <div class="workitem-info-bar">
                    <div class="info-text-wrapper"><span class="info-text">Danh sách người dùng</span></div>
                </div>
                <div class="toolbar workitem-tool-bar" style="display: block;">
                    <ul class="menu-bar">

                        <li class="menu-item icon-only" id="btn-tinh-chat-xuat-nhap">
                            <a href="default.aspx?page=adduser"><span class="icon icon-add"></span><span class="text">Thêm người dùng</span></a>
                        </li>
                        ...
                        <li class="menu-item right-align icon-only">
                            ...
                            <div class="search-adapter-work-items header-search search-box">
                                ...
                                <asp:TextBox ID="txtTimKiem" nams="s" runat="server" Width="350px" placeholder="Tìm kiếm theo tên đăng nhập hoặc email" CssClass="box-search-in-form"></asp:TextBox>
                    ...
                    <table class="data-table" id="tabDanhSachPhieuNhapVatTu">
                        <thead>
                            <tr>
                                ...
                                <td>Kích hoạt</td>
                                <td>Khóa</td>
                                <%--<td style="width: 110px;">Đang đăng nhập</td>--%>
                                <td style="width: 130px;">Đăng nhập gần nhất</td>
                            ...
                            <asp:Repeater ID="rptData" runat="server" OnItemDataBound="rptData_ItemDataBound">
                                <ItemTemplate>
                                    <tr>
                                        ...
                                        <td><%# Eval("UserName") %></td>
                                        <td style="width: 160px;"><%# Eval("Email") %></td>
                                        <td class="center-col">
                                            <asp:Label runat="server" ID="lblActive" Visible="false"> </asp:Label>
                                            <%#Util.RerenderStatus(Eval("IsApproved"),"Đã kích hoạt","Chưa kích hoạt") %></td>
                                        <td class="center-col">
                                            <asp:Label runat="server" ID="lblLock"></asp:Label>
                                            <%#Util.RerenderStatus(Eval("IsLockedOut"),"Bị khóa","Đang hoạt động") %></td>
                                        </td>
                                        <%--<td style="width: 110px;" class="center-col">
                                            <asp:Label runat="server" ID="lblOnline"></asp:Label>
                                        </td>--%>
                                        <td style="width: 130px;"><%# string.Format("{0:dd/MM/yyyy}", Eval("LastLoginDate")) %></td>
                                        <td class="center-col" style="width: 90px;">
                                            <a class="edit-icon" id="edit-icon" href="default.aspx?page=edituser&id=<%# Eval("UserName")%>"></a>
                                            <a class="delete-icon" href="javascript:deleteItem('<%# Eval("UserName")%>');"></a>
                                        </td>
                                    </tr>
                                </ItemTemplate>
                            </asp:Repeater>
