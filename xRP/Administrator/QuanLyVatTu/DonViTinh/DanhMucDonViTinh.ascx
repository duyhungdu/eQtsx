<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DanhMucDonViTinh.ascx.cs" Inherits="xRP.Administrator.QuanLyVatTu.DonViTinh.DanhMucDonViTinh" %>
<%@ Register Assembly="CollectionPager" Namespace="SiteUtils" TagPrefix="cc1" %>
<script type="text/javascript">
    function OpenModal(id) {
        if (id=="-1")
            $("#content_div").prop('title', 'Thêm mới đơn vị tính');
        else
            $("#content_div").prop('title', 'Hiệu chỉnh đơn vị tính');
        LoadUserControl("<%=Util.SERVER_NAME %>/Default.aspx", "/Administrator/QuanLyVatTu/DonViTinh/AddOrEditDonViTinh.ascx", "&id=" + id);
        $("#content_div").dialog({ modal: true, width: 400, height: 200 });
    }
    function CloseModal() {
    ...
    function deleteItem(id) {
        var answer = confirm("Bạn có thực sự muốn xóa \"" + "đơn vị tính" + "\"?")
        if (answer) {
            $.ajax({
                type: "POST",
                url: "<%#Util.SERVER_NAME %>/Administrator/QuanLyVatTu/DonViTinh/Default.aspx/DeleteItem",
                data: "{itemID:" + id + "}",
        ...
        <div class="right-hub-content">
            <div class="work-item-form" style="display: block;">
                <div class="workitem-info-bar">
                    <div class="info-text-wrapper"><span class="info-text">Danh sách đơn vị tính</span></div>
                </div>
                <div class="toolbar workitem-tool-bar" style="display: block;">
                    <ul class="menu-bar">
                        <li class="menu-item icon-only" id="btn-tao-phieu-nhap-kho" onclick="OpenModal(-1);"><span class="icon icon-add"></span><span class="text">Thêm đơn vị tính</span></li>
