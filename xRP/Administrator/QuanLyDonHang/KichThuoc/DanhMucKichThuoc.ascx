<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DanhMucKichThuoc.ascx.cs" Inherits="xRP.Administrator.QuanLyDonHang.KichThuoc.DanhMucKichThuoc" %>
<%@ Register Assembly="CollectionPager" Namespace="SiteUtils" TagPrefix="cc1" %>
<script type="text/javascript">
    function OpenModal(id) {
        var paging = $.QueryString("trang");
        if (paging != null) {
            LoadUserControl("<%=Util.SERVER_NAME %>/Default.aspx", "/Administrator/QuanLyDonHang/KichThuoc/AddOrEditKichThuoc.ascx", "&id=" + id + "&trang=" + paging);
        }
        else {
            LoadUserControl("<%=Util.SERVER_NAME %>/Default.aspx", "/Administrator/QuanLyDonHang/KichThuoc/AddOrEditKichThuoc.ascx", "&id=" + id);
        }
        $("#content_div").dialog({ modal: true, width: 900, height: 300 });
    }
    function CloseModal() {
    ...
    function xoa(id) {
        ...
        if (answer) {
            ...
            $.ajax({
                type: "POST",
                url: "<%#Util.SERVER_NAME %>/Administrator/QuanLyDonHang/Default.aspx/XoaSong",
                data: "{id:" + id + "}",
        ...
        <div class="right-hub-content">
            <div class="work-item-form" style="display: block;">
                <div class="workitem-info-bar">
                    <div class="info-text-wrapper"><span class="info-text">Danh sách kích thước</span></div>
                </div>
                <div class="toolbar workitem-tool-bar" style="display: block;">
                    <ul class="menu-bar">

                        <li class="menu-item icon-only" id="btn-tao-moi-san-pham" onclick="OpenModal(-1);"><span class="icon icon-add"></span><a href="javascript:void(0);">Tạo mới kích thước</a></li>
                ...
                <div class="work-item-view" style="display: block;">
                    <table class="data-table">
                        ...
                        <tbody>
                         <asp:Repeater ID="rptDanhMucKichThuoc" runat="server">
                                <ItemTemplate>
                            <tr>
                                <td class="center-col" style="width:5%"><%# (colPage.CurrentPage - 1) * colPage.PageSize + Container.ItemIndex + 1 %></td>
                                <td><%# Eval("MaHieuKichThuoc") %></td>
                                <td><%# Eval("LoaiKichThuoc") %></td>
                                <td><%# Eval("HeSoChatDai") %></td>
                                <td><%# Eval("KichHoatText") %></td>
...
<div id="content_div" title="Cập nhật danh mục kích thước">
</div>