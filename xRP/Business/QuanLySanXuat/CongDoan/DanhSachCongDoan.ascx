<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DanhSachCongDoan.ascx.cs" Inherits="xRP.Business.QuanLySanXuat.CongDoan.DanhSachCongDoan" %>
        ...
        <div class="right-hub-content">
            <div class="work-item-form" style="display: block;">
                <div class="workitem-info-bar">
                    <div class="info-text-wrapper"><span class="info-text">Danh sách công đoạn sản xuất</span></div>
                </div>
                <div class="toolbar workitem-tool-bar" style="display: block;">
                    <ul class="menu-bar">

                        <li class="menu-item icon-only" id="btn-tinh-chat-xuat-nhap" onclick="OpenModal(-1);"><span class="icon icon-add"></span><span class="text">Thêm công đoạn sản xuất</span></li>
                    ...
                    <table class="data-table" id="tabDanhSachPhieuNhapVatTu">
                        <thead>
                            <tr>
                                ...
                                <td style="width: 7%;">Mã hiệu</td>
                                <td style="width: 15%;">Tên công đoạn</td>
...
<div id="content_div" style="z-index: 1002;" title="Cập nhật công đoạn sản xuất">
</div>
<script type="text/javascript">
    function OpenModal(id) {
        var paging = $.QueryString("trang");
        if (paging != null) {
            LoadUserControl("<%=Util.SERVER_NAME %>/Default.aspx", "/Business/QuanLySanXuat/CongDoan/AddOrEditCongDoan.ascx", "&id=" + id + "&trang=" + paging);
        }
        else {
            LoadUserControl("<%=Util.SERVER_NAME %>/Default.aspx", "/Business/QuanLySanXuat/CongDoan/AddOrEditCongDoan.ascx", "&id=" + id);
        }
        $("#content_div").dialog({ modal: true, width: 700, height: 400 });
    }
    function deleteCongDoan(congDoanId) {
        var data = new Object();
        data.itemID = congDoanId;
        data.maDonViPhuTrach = 0;
        data.kichHoat = true;
        data.tenCongDoan = "";
        data.maHieuCongDoan = "";
        data.ghiChu = "";
        var delCongDoan = new CongDoan(data);
        delCongDoan.markDelete(function (result) {
            var newid = result.id;
            if ($.isNumeric(newid)) {
